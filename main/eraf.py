...
from utils import select_data, set_seed, Save, get_proto, get_max_sim, convert_to_probability_list
from config import get_config
from utils import classification_report, f1_score, confusion_matrix_view, get_aca_data
import multiprocessing

from model.eraf import Bert_Encoder, Attention_Memory_Simplified, Softmax_Layer

logger = logging.getLogger(__name__)


def evaluate_strict_model(config, test_data, seen_relations, rel2id, mode="cur", logger=None, pid2name=None, rp_cre_input={}):
    data_loader = get_data_loader(config, test_data, batch_size=140)
    n = len(test_data)
    ...
    with torch.no_grad():
        for _, (_, labels, tokens, _, _) in enumerate(tqdm(data_loader, desc="Evaluate {}".format(mode))):
            ...
            per_correct = ((predicts == labels).sum().item()) / 140
            relation_correct[seen_relations[relation_seq]] = per_correct
            relation_seq += 1
    ...
    return correct / n, relation_correct
...
def main():
    ...
    log_path = os.path.join(config.log_dir, "{}".format(config.exp_name) + '0.6.txt')
    ...
    for i in range(config.total_round):
        ...
        for episode, (training_data, _, test_data, current_relations,
            ...
            if len(seen_relations) == len(current_relations):
                select_num = M_SIZE // len(current_relations)
            else:
                select_num = M_SIZE // (len(seen_relations) - len(current_relations))
            for relation in current_relations:
                temp_mem[relation] = select_data(config, encoder, training_data[relation], select_num)
            ...
            n = int((M_SIZE // len(seen_relations)) * 1.2)


            def random_from_dict_by_value(input_dict):
                filtered_data = {k: v for k, v in input_dict.items() if v >= 0.5}
                total = sum(filtered_data.values())
                probabilities = [value / total for value in filtered_data.values()]
                random_sample = random.choices(list(filtered_data.keys()), probabilities)[0]
                random_sample_value = data[random_sample]


            new_selected_relations = []
            n = int((M_SIZE // len(seen_relations)) * 1.2)
            for relation in current_relations:
                memorized_samples[relation] = select_data(config, encoder, training_data[relation], n)
                new_selected_relations += memorized_samples[relation]

            if len(seen_relations) == len(current_relations):
                print("fill")
            else:
                old_memorized_relations = []
                for relation in seen_relations:
                    if relation not in current_relations:
                        old_memorized_relations += memorized_samples[relation]
                new_inner_sim = get_max_sim(config, encoder, new_selected_relations)
                old_inner_sim = get_max_sim(config, encoder, old_memorized_relations)
                for index_n, item in tqdm(enumerate(new_selected_relations), total=len(new_selected_relations)):
                    score_old = []
                    for index, value in enumerate(old_memorized_relations):
                        if id2rel[value["relation"]] in current_relations:
                            score_old.append(old_inner_sim[index])
                        else:
                            score_old.append((old_inner_sim[index] * 0.4 + relation_correct[id2rel[value["relation"]]] * 0.6))
                    probability_old = convert_to_probability_list(score_old)
                    sample = np.random.choice(old_memorized_relations, p=probability_old)
                    sample_index = old_memorized_relations.index(sample)
                    a = random.random()
                    if a < score_old[sample_index] / (score_old[sample_index] + new_inner_sim[index_n]):
                        old_memorized_relations.remove(sample)
                        old_memorized_relations.append(item)
                        old_inner_sim = get_max_sim(config, encoder, old_memorized_relations)

                for relation in seen_relations:
                    memorized_samples[relation] = []

                for x in old_memorized_relations:
                    memorized_samples[id2rel[x["relation"]]].append(x)
            ...
            cur_acc, _ = evaluate_strict_model(config, test_data_1,seen_relations, rel2id, mode="cur", pid2name=pid2name,  rp_cre_input=rp_cre_input)
            total_acc, relation_correct = evaluate_strict_model(config, test_data_2 ,seen_relations, rel2id, mode="total", logger=logger, pid2name=pid2name, rp_cre_input=rp_cre_input)
