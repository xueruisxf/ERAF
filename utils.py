...
def get_max_sim(config, encoder, sample_set):
    data_loader = get_data_loader(config, sample_set, shuffle=False, drop_last=False, batch_size=1)
    features = []
    ...
    index = faiss.IndexFlatIP(len(features[0]))
    index.add(features)
    cos_sims, indexes = index.search(features, k=2)
    #
    cos_scores = [np.min(cos_sims[i]) for i in range(len(features))]

    # new_set = []
    # for k in range(len(features)):
    #     instance = sample_set[k]
    #     instance["score"] = cos_scores[k]
    #     new_set.append(instance)
    return cos_scores


def convert_to_probability_list(nums):
    total_sum = sum(nums)
    probability_list = [num / total_sum for num in nums]
